...
edad <- factor(c(rep("14-24", 12.46), rep("25-39", 28.77), rep("40-49", 13.14), rep("50-64", 22.30), rep("65+", 23.33)))


PieChart(edad, hole = 0, values = "%", data = edad,
...
```{r}
rango_trabajo_pago <- range (uso_tiempo$trabajo_pago, na.rm = TRUE)

```
 

La variable trabajo pago, que indica la cantidad de horas diarias dedicadas a tareas remuneradas toma valores en  `r rango_trabajo_pago [1]` y `r rango_trabajo_pago [2]`.

Rango para trabajo doméstico no pago.
```{r}

Rango_domestico_no_pago <- range(uso_tiempo$trabajo_domestico_no_pago)

```

La variable tareas domésticas no pagas, que indica la cantidad de horas diarias dedicadas a tareas en el hogar no remuneradas, toma valores entre  `r Rango_domestico_no_pago [1]` y `r Rango_domestico_no_pago [2]`.


```{r}
rango_voluntariado <- range(uso_tiempo$voluntariados)

```

La variable voluntariado, que indica la cantidad de horas diarias dedicadas a tareas de carácter voluntario, toma valores entre  `r rango_voluntariado [1]` y `r rango_voluntariado [2]`.


Rango para tareas de cuidados

```{r}
rango_cuidados <- range(uso_tiempo$tareas_cuidados)
```

La variable cuidados, que indica la cantidad de horas diarias dedicadas a tareas de cuidados de familiares  toma valores entre  `r rango_cuidados [1]` y `r rango_cuidados [2]`.

Rango para tareas uso de aplicaciones

```{r}
rango_aplicaciones <- range(uso_tiempo$aplicaciones)

```

La variable aplicaciones, que denota la cantidad de horas diarias dedicadas al uso de aplicaciones tecnológicas,  toma valores entre  `r rango_aplicaciones [1]` y `r rango_aplicaciones [2]`. 


Rango para tareas tiempo de recreación 

```{r}
rango_recreacion <- range(uso_tiempo$recreacion)

```

La variable recreación, expresa la cantidad de horas diarias dedicadas al ocio y recreación,  toma valores entre  `r rango_recreacion [1]` y `r rango_recreacion [2]`. 


Se observa que los limites superiores resultan extraños, si están expresado en minutos. 
...
PieChart(genero, hole = 0, values = "%", data = genero,
         ...
         fill = c("green", "blue"), main = "Distribución porcentual según sexo") 
```
...
2. El 57% de las respuestas provienen del genero femenino.

3. No es posible utilizar la variable quintil de ingreso.

4. EL 37% de los encuestas posse superior completo, 42% superior incompleto y un 21% secundario incompleto.


 
## Hipótesis 1: Las mujeres dedican mayor cantidad de horas al trabajo no remunerado que los hombres.

Para ver ello, se calcula la cantidad media de tiempo dedicada al trabajo pago en el día de referencia según sexo. Lo mismo para aquella actividades no remuneradas realizadas. Para el segundo grupo se considerarón las siguientes variables:  trabajo doméstico no remunerado, cuidados y voluntariado.
...
horas_no_pago <- uso_tiempo %>% 
  mutate (hs_no_pago =(trabajo_domestico_no_pago+voluntariados+tareas_cuidados)) 
...
En promedio las mujeres realizan más horas de trabajo no remunerado que los hombres. Es decir, mientras que los hombres en el dia de referencia realizaron poco más de 3 horas de tareas no remuneradas,las mujeres hicieron más de 5 horas promedio.

Es importante destacar que el desvio es elevado, en el caso de las mujeres si bien en promedio realizan tareas no remuneradas 5 horas en el día de referencia existe un desvio de más/menos 4 horas diarias. Para los hombres dicho desvio es cercano a 3 horas.
...
A continuación se realiza el mismo análisis desagregando por genero/sexo.
...
Cuando este análisis se lo discrimina por género, se observa que las mujeres ocupadas realizan más actividades no remuneradas que los hombres en igual condición laboral,cerca de 5 horas versus poco menos de 3 respectivamente.



## Hipotesis 3: Las personas más jovenes realizan menos actividades no remuneradas y más recreativas o de estudio.
...
horas_no_pago %>% 
  ...
  summarise(media_no_pago = mean(hs_no_pago, na.rm = TRUE), horas_estudio = mean(estudio_educacion),horas_recreacion = mean(recreacion, na.rm = TRUE),horas_aplicaciones = mean(aplicaciones), horas_pago = mean(trabajo_pago)) %>%
  knitr::kable(caption = "Promedio de horas de recreación o estudio según edad ", col.names = c("Edad agrupada ", "Horas de trabajo no remunerado", "Horas de estudio", "Horas de recreación", "Horas en aplicaciones","Horas de trabajo remunerado"))
...
uso_tiempo|>
  group_by(rango_etario)|>
  summarise(media_recreacion = mean(recreacion, na.rm=TRUE))|>
...
uso_tiempo|>
  group_by(rango_etario)|>
summarise(media_aplicaciones = mean(aplicaciones, na.rm=TRUE))|>
...
uso_tiempo|>
  group_by(rango_etario)|>
summarise(media_estudio = mean(estudio_educacion, na.rm=TRUE))|>
